...
public class DownloadManager {
    ...
    public static String getApkName(Context context) {
        return TextUtils.isEmpty(apkName) ? context.getPackageName() : apkName;
    }
    ...
    public static void downloadAPK(final Context context, String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        apkName = TextUtils.isEmpty(versionParams.getApkName()) ? context.getPackageName() : versionParams.getApkName();
...
//        ApkBroadCastReceiver.downloadApkPath = versionParams.getDownloadAPKPath();
        if (!versionParams.isForceRedownload()) {
            //判断本地文件是否存在
            String downloadPath = versionParams.getDownloadAPKPath() + context.getString(R.string.versionchecklib_download_apkname, apkName);
        ...
        AllenHttp.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), context.getString(R.string.versionchecklib_download_apkname, apkName)) {
            @Override
